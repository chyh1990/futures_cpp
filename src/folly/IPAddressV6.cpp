...
void IPAddressV6::setFromBinary(ByteRange bytes) {
  if (bytes.size() != 16) {
    throw IPAddressFormatException("Invalid IPv6 binary data: length must "
                                   "be 16 bytes, got " + std::to_string(bytes.size()));
  }
...
string IPAddressV6::toJson() const {
  return
      "{family:'AF_INET6', addr:'" + str() + "', hash:"+ std::to_string(hash()) +"}";
}
...
size_t IPAddressV6::hash() const {
  ...
  return hash::fnv64_buf(&addr_, 16);
...
bool IPAddressV6::inSubnet(StringPiece cidrNetwork) const {
  ...
  if (!addr.isV6()) {
    ...
    //                                "is not a V6 address");
    throw IPAddressFormatException("Address is not a V6 address");
  }
...
IPAddressV6 IPAddressV6::mask(size_t numBits) const {
  ...
  if (numBits > bits) {
    throw IPAddressFormatException("numBits(" + std::to_string(numBits) + ") > bitCount(" +
                                   std::to_string(bits) + ")");
  }
