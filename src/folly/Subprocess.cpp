...
#include <futures/core/Subprocess.h>
...
static void throwSystemError(int err, const std::string &s) {
  throw std::system_error(err, std::system_category(), s);
}

static void checkUnixError(int ret, const char *s) {
  if (ret == -1)
    throwSystemError(s);
}

static void checkUnixError(int ret, int err, const char *s) {
  if (ret == -1)
    throwSystemError(err, s);
}

static void checkPosixError(int ret, const char *s) {
  if (ret == 0)
    throwSystemError(s);
}
...
ProcessReturnCode::State ProcessReturnCode::state() const {
  ...
  throw std::runtime_error(std::string(
      "Invalid ProcessReturnCode: ") + std::to_string(rawStatus_));
}
...
void ProcessReturnCode::enforce(State expected) const {
  ...
  if (s != expected) {
    throw std::logic_error(
      "Bad use of ProcessReturnCode; state is " + std::to_string(s) + " expected " + std::to_string(expected)
    );
  }
}
...
SubprocessSpawnError::SubprocessSpawnError(const char* executable,
  ...
  : errnoValue_(errnoValue),
    what_(errCode == kExecFailure ?
                            "failed to execute " :
                            "error preparing to execute " +
                          std::string(executable) + ": " + std::to_string(errnoValue)) {
}
  ...
  if (action == Subprocess::PIPE) {
    if (fd == 0) {
      ...
      throw std::invalid_argument(
          "Only fds 0, 1, 2 are valid for action=PIPE: " + std::to_string(fd));
    }
  }
...
size_t Subprocess::findByChildFd(int childFd) const {
  ...
  if (pos == pipes_.end() || pos->childFd != childFd) {
    throw std::invalid_argument(
        "child fd not found " + std::to_string(childFd));
  }
