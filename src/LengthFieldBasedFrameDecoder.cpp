...
#include <futures/codec/LengthFieldBasedFrameDecoder.h>
#include "folly/GlogDummy.h"
...
Optional<LengthFieldOut> LengthFieldBasedFrameDecoder::decode(folly::IOBufQueue& buf) {
  ...
  if (frameLength > maxFrameLength_) {
    ...
    throw std::runtime_error("Frame larger than " + std::to_string(maxFrameLength_));
  }
  ...
  return Out(buf.split(actualFrameLength));
}
