...
inline bool getNthMSBitImpl(const IPAddrType& ip, uint8_t bitIndex,
  ...
  if (bitIndex >= ip.bitCount()) {
    throw std::invalid_argument("Bit index must be < " +
          std::to_string(ip.bitCount())
          + " for addresses of type :" + familyNameStr(family));
  }
  ...
  longestCommonPrefix(
    ...
    if (oneMask > kBitCount || twoMask > kBitCount) {
      throw std::invalid_argument("Invalid mask "
            "length: " + std::to_string(oneMask > twoMask ? oneMask : twoMask)
            + ". Mask length must be <= " + std::to_string(kBitCount));
    }
