...
#include <futures/core/IPAddressV4.h>

#include <ostream>
#include <string>

#include <futures/core/IPAddress.h>
#include <futures/core/IPAddressV6.h>
...
uint32_t IPAddressV4::toLong(StringPiece ip) {
  ...
  if (inet_pton(AF_INET, str.c_str(), &addr) != 1) {
    throw IPAddressFormatException("Can't convert invalid IP '" + str + "' to long");
  }
  return addr.s_addr;
}
  ...
  : addr_()
{
  ...
  if (inet_pton(AF_INET, ip.c_str(), &addr_.inAddr_) != 1) {
    throw IPAddressFormatException("Invalid IPv4 address '" + ip + "'");
  }
}
...
void IPAddressV4::setFromBinary(ByteRange bytes) {
  if (bytes.size() != 4) {
    throw IPAddressFormatException("Invalid IPv4 binary data: length must be 4 bytes, got "
...
string IPAddressV4::toJson() const {
  return
      "{family:'AF_INET', addr:'" + str() + "', hash:"+ std::to_string(hash()) +"}";
}
...
bool IPAddressV4::inSubnet(StringPiece cidrNetwork) const {
  ...
  if (!addr.isV4()) {
    ...
    //                                "is not a V4 address");
    throw IPAddressFormatException("Address is not a V4 address");
  }
...
IPAddressV4 IPAddressV4::mask(size_t numBits) const {
  ...
  if (numBits > bits) {
    throw IPAddressFormatException("numBits(" + std::to_string(numBits) +
                                   ") > bitsCount(" + std::to_string(bits) + ")");
  }
