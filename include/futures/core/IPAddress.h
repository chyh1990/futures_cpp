...
#include <futures/core/IPAddressException.h>
#include <futures/core/IPAddressV4.h>
#include <futures/core/IPAddressV6.h>
#include <futures/core/detail/IPAddress.h>
...
class IPAddress : boost::totally_ordered<IPAddress> {
 public:
  ...
  const IPAddressV6& asV6() const {
    if (!isV6()) {
      auto familyName = detail::familyNameStr(family());
      throw InvalidAddressFamilyException("Can't convert address with family " + familyName + " to AF_INET6 address");
    }
